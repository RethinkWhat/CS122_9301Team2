...
public class FractionTester extends JFrame {
    ...
    /*
        Process:
            ...
            input. Then we create a String array object called fractionString with length 3 to hold the whole number,
            numerator, and denominator that would be inputted by the user in the JTextField. We make use of three
            different try and catch statements to handle the 3 possible inputs by a user a whole number, a fraction, and
            a mixed fraction.

            The first try and catch splits the input of the user such that it will be split after a
            space and a slash -> input.split(" |/"). After the seperation is done, it assigns the value to
            fractionString with [0] being the index for wholeNumber, [1] numerator, and [2] denominator respectively.
            Using these indexes are assigned to the variables wholeNumber, numerator, and denominator declared earlier.
            We make use of the Integer.valueOf because the values are still in String format after the split was done earlier.

            Next we move on to the second try and catch which makes use of input.split("/"). This means there will only
            be one split and it will be exactly where the slash is placed. The remaining processes are the same in this
            try and catch except that wholeNumber will be assigned the value of 0.

            Last is the possibility that a whole number was entered. In this possibility we do not need to split
            anything. All we do is assign the value to wholeNumber, set denominator to 1 and numerator to 0.

            After which we create a MixedFraction object assign it the appropriate values and return it
